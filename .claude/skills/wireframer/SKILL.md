...
Every wireframe belongs to a feature and lives in that feature's own folder:

```
outputs/<feature-name>/wireframes/
```

Rules:

1. **Determine the feature.** Use the feature the user named. If they didn't name one, infer it from the conversation or the active artifact, and use a kebab-case slug (`connected-sites-graph`, `mcc-mismatch`).
2. **Reuse the existing feature folder** if `outputs/<feature-name>/` already exists. Never create a second folder for the same feature under a different slug.
3. **Create `outputs/<feature-name>/wireframes/`** and write the prototype there — `index.html` for a single-screen or SPA prototype, or one file per screen plus an `index.html` entry point for multi-screen sets.
4. **Never write wireframes to the repo root, a scratch directory, or `design/`.** Never modify `CLAUDE.md`, `agents.md`, or any other project instruction file — the aesthetic rules live in this skill, not in project context files.
5. **Add a short `README.md`** in the `wireframes/` folder: what the wireframe shows, which screens exist, what open questions it is meant to answer, and the date.
6. **Report the path** to the user when done.
...
Resolve the feature folder per §1, then generate the requested prototype without further preamble. Do not ask clarifying questions — pick the most reasonable interpretation, build it, and state assumptions in one line at the end.
